...
public class InicioController implements Initializable {
	...
	private void quitarElementos(ActionEvent event) {
		...
		if (!Dialogos.confirm("緽orrar archivos y registros seleccionados?")) {
			return;
		}
		try (Connection conn = proyecto.getDataSource().getConnection()) {
		...
		for (Procedimiento r : tabla_sps.getSelectionModel().getSelectedItems()) {
			...
			try {
				if (flocal.exists() && !Dialogos
						.confirm("驴Reemplazar versi贸n de ClearCase con la versi贸n del directorio local?")) {
					return;
				}
	...
	private void copiarDesdeClearCase() {
		for (Procedimiento r : tabla_sps.getSelectionModel().getSelectedItems()) {
			...
			try {
				if (flocal.exists()
						&& !Dialogos.confirm("驴Reemplazar versi贸n local con la versi贸n del directorio ClearCase?")) {
					return;
				}
	...
	class ProyectoReader extends Task<Proyecto> {
		...
		protected Proyecto call() {
			...
			try {
				...
				updateMessage("config.xml no existe o es inv醠ido");
				Logger.getLogger(InicioController.class.getName()).log(Level.WARNING,
						"No fue posible cargar configuraci髇 de " + p.toString(), ex);
				return null;
			}
		}
	}
